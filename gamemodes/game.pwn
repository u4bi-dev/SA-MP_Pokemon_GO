...
stock textLabel_init(){
  for(new a = 0;a<3;a++){
    new str[40];
    format(str, sizeof(str),"%s (Y??",MissonDTO[a][NAME]);
...
stock joinController(playerid){
  ...
  if(rows){
    ...
    ShowPlayerDialog(playerid, d_Log, DIALOG_STYLE_PASSWORD, "{8D8DFF}怨꾩젙愿由?留ㅻ땲?", "{FFFFFF}鍮꾨?踰덊샇瑜??낅젰?댁＜?몄슂.", "濡쒓렇??, "?섍?湲?);
  }else{
    ShowPlayerDialog(playerid, d_Reg, DIALOG_STYLE_PASSWORD, "{8D8DFF}怨꾩젙愿由?留ㅻ땲?", "{FFFFFF}鍮꾨?踰덊샇瑜??낅젰?댁＜?몄슂.", "?뚯썝媛??, "?섍?湲?);
  }
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]){
  ...
  switch(dialogid){
      ...
      case poketmon_Bag:{
      if(PoketmonDTO[playerid][0][ID]==0) return SendClientMessage(playerid,col_sys,"		?ъ폆紐ъ씠 ?놁뒿?덈떎.");
      bagDialog(playerid,response,listitem,0);
    }
      case player_Bag:{
      if(ItemDTO[playerid][0][ID]==0) return SendClientMessage(playerid,col_sys,"		?꾩씠?쒖씠 ?놁뒿?덈떎.");
      bagDialog(playerid,response,listitem,1);
    }
      case slot_bag:slotEventItem(playerid,response,listitem);
      case car_Bag:{
      if(CarDTO[playerid][0][ID]==0) return SendClientMessage(playerid,col_sys,"		李⑤웾???놁뒿?덈떎.");
...
stock mediDialog(playerid,response,listitem,type){
  ...
  if(response){
    ...
    ShowPlayerDialog(playerid, misson_medi, DIALOG_STYLE_LIST,str,"{FFFFFF}?ъ폆紐?移섎즺\n?ъ폆紐??⑹꽦\n?ъ폆紐?遺꾩뼇\n????섏뒪??","?뺤씤", "痍⑥냼");
  }
}
...
stock itemShopDialog(playerid,response,listitem,type){
  ...
  if(response){
    ...
    ShowPlayerDialog(playerid, misson_itemshop, DIALOG_STYLE_LIST,str,"{FFFFFF}?꾩씠??援щℓ\n?꾩씠???먮ℓ\n????섏뒪??","?뺤씤", "痍⑥냼");
  }
}
...
stock carShopDialog(playerid,response,listitem,type){
  ...
  if(response){
    ...
    ShowPlayerDialog(playerid, misson_carshop, DIALOG_STYLE_LIST,str,"{FFFFFF}李⑤웾 援щℓ\n李⑤웾 ?먮ℓ\n????섏뒪??","?뺤씤", "痍⑥냼");
  }
}
...
stock takeItem(playerid,num){
  switch(num){
      case 0..3:
      {
      if(IngameDTO[playerid][ISBALL] == true) return SendClientMessage(playerid,col_sys,"		?대? 紐ъ뒪?곕낵??爰쇰궡?⑥뒿?덈떎.");
...
stock putItem(playerid,num){
  switch(num){
      case 0..3:
      {
      if(IngameDTO[playerid][ITEAMNUM] != num) return SendClientMessage(playerid,col_sys,"		紐ъ뒪?곕낵??爰쇰궡吏 ?딆븯?듬땲??");
      if(IngameDTO[playerid][ISBALL] == false) return SendClientMessage(playerid,col_sys,"		紐ъ뒪?곕낵??爰쇰궡吏 ?딆븯?듬땲??");
  ...
  switch(type){
    case 0:{
      if(!strcmp(inputtext, UserDTO[playerid][PASS])){
        ...
        }else ShowPlayerDialog(playerid, d_Log, DIALOG_STYLE_PASSWORD, "{8D8DFF}怨꾩젙愿由?留ㅻ땲?", "{FFFFFF}鍮꾨?踰덊샇媛 ?由쎈땲??", "濡쒓렇??, "?섍?湲?);
    }
    case 1:{
          if(strlen(inputtext) > 24) return ShowPlayerDialog(playerid, d_Reg, DIALOG_STYLE_PASSWORD, "{8D8DFF}怨꾩젙愿由?留ㅻ땲?", "{FFFFFF}24???댄븯??鍮꾨?踰덊샇瑜??낅젰?댁＜?몄슂.", "?뚯썝媛??, "?섍?湲?);
        if(strlen(inputtext) < 6) return ShowPlayerDialog(playerid, d_Reg, DIALOG_STYLE_PASSWORD, "{8D8DFF}怨꾩젙愿由?留ㅻ땲?", "{FFFFFF}6???댁긽??鍮꾨?踰덊샇瑜??낅젰?댁＜?몄슂.", "?뚯썝媛??, "?섍?湲?);
...
/*HACK : 데이터 초기화 추후 옵티마이징 작업을 위한 코드리펙해야함*/
...
stock checkBall(playerid, Float:distance,Float:ballDistance){
  if(distance > ballDistance) return SendClientMessage(playerid,col_sys,"		二쇰????ъ폆紐ъ씠 ?놁뒿?덈떎.");
  new str[50];
...
stock rideCar(playerid,num,model){
  if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid,col_sys,"		?대? 李⑤웾???묒듅以묒엯?덈떎.");
  new Float:pos[4];
  GetPlayerPos(playerid, pos[0], pos[1], pos[2]);
  GetPlayerFacingAngle(playerid, pos[3]);
  switch(num){
